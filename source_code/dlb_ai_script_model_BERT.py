...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  9 17:28:17 2021
...
possible_labels = np.sort(df.agility.unique())
...
df['label'] = df.agility.replace(label_dict).astype(int)
df.head()
...
df.groupby(['agility', 'label', 'data_type']).count()

df.dtypes

df_col_len = int(df['text'].astype(bytes).str.len().max())
...
for epoch in tqdm(range(1, epochs+1)):
    ...
    torch.save(model.state_dict(), f'/Users/Roland/Downloads/finetuned_BERT_epoch_DLB_512_{epoch}.model')
...
model.load_state_dict(torch.load('/Users/Roland/Downloads/finetuned_BERT_epoch_DLB_512_1.model'
